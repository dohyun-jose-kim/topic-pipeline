...
## 7. ver3.0.2 — 범용 tool 델타 max-effort 코드리뷰 결과 (2026-06-27)

발행 후 추가된 범용 tool 기능(`fd7fceb..HEAD`: LLM provider/local, keywords fallback,
ingest 어댑터, --init/--preset, --serve, Docker)을 다각도(10 finder→verify→sweep) 리뷰.
아래 4건은 **3.0.2 에서 해결**, #12 는 기능성 추가라 후속으로 기록.

- **✅ #8 (HIGH) serve `logs/` 가드레일 우회** — `_serve_blocked` 가 raw URL 을 검사하고
  `translate_path` 는 그 뒤 unquote+normpath 하던 불일치로 `/%6Cogs/`·`/logs%2f`·`/./logs/`·
  `/foo/../logs/`·`/Logs/`·하위 run-id 경로가 DEBUG 로그를 서빙(+`do_HEAD` 미가드, 한글 reason
  latin-1 크래시). → unquote→normpath 후 'logs' 세그먼트 차단 + GET·HEAD 공통 `send_head`
  단일 차단점 + explain 본문 이동. 라이브 소켓 6벡터 검증. (커밋 `3575dc8`)
- **✅ #9 (MED) call_local 응답 shape 미검증** — `{choices:[]}`/`content=null` 이 retry 밖
  IndexError/None 로 크래시 → 명확한 RuntimeError(본문 비노출). (커밋 `90887ac`)
- **✅ #10 (MED) _run_arxiv 재시도 부재** — 단발 GET 이 503 한 번에 중단 → repo 전역 패턴대로
  지수 backoff 재시도. (커밋 `e3bbbdd`)
...
**잔여:** #15 재현성(seed 근사·캐시 핀, 문서화). **C(실데이터 5590)** 는 §10 에서 로컬 LLM 으로
실행 완료 — Claude 라벨 경로(provider=claude)만 사용자 `ANTHROPIC_API_KEY` 필요(미실행).

---

## 10. 실데이터 5590 + provider=local(Ollama) 라이브 검증 (2026-06-27)

§9(C) 후속 — 사용자 환경의 Ollama 로 **API 키 없이** 실데이터 풀런 + 로컬 LLM 라벨 경로를 라이브
검증(이전까지 mock 단위테스트만 있던 `provider=local`·`generate` 경로의 실전 확인).

- **✅ provider=local 라이브** (이전 "provider=local/claude live A/B 필요" 항목 해소): `call_local` →
  Ollama OpenAI 호환 엔드포인트. exaone3.5:32b 로 합성 코퍼스 + 실데이터 모두 s5 라벨/관련도 생성
  성공. #9(응답 shape 검증)·#12(Topic 드리프트 경고) 실전 통과 — 크래시·누락 0.
- **✅ 실데이터 5590 풀런** (`default_config` + provider 만 local): PubMed 5590편 fetch(58s) →
  PubMedBERT CPU 임베딩(247s) → 기본 8-point sweep → exaone3.5:32b 라벨 → s7. 총 9:25, exit 0.
  생존자 `[27,37,71]` → median_low **mts=37 → 16토픽**(KNOWN §4 의 과거 5토픽 과병합보다 개선).
  **invariant#3 준수**(16토픽 전부 정수 파싱, 드리프트 경고 0), s7_report.html 10.8MB.
- 라벨 품질: 도메인 정확(t0 Oxidative Stress in Hepatopancreas · t3 Chitosan Antimicrobial
  Applications · t9 Fish Collagen Extraction 등). 한글 음역 일부 garble 은 exaone 모델 특성(도구 무관).
- **미실행:** Claude 라벨 경로(provider=claude)는 `ANTHROPIC_API_KEY` 필요. 검증 런은 scratchpad
  전용 — repo 코드·추적 산출물 불변(버전 변동 없음).
